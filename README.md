...
`Tip: if you need to find missing number from a list of continuous numbers, perform: expected sum - actual sum`
...
`Tip: when solving a problem with approach recussion(take) + recussion(skip), we must remember to add a stopping condition on number of attempts. This is true only is the range is infinite. To explain this with an example: lets say we need to check if we can form a value using powers of 2, i.e., 2^0, 2^1, 2^2, ... 2^inf. Because the range here is infinite, we must add a stopping condition like if 2 ^ i > given_value, return False. This is because we will always have a path where we skipped from the very start and this path will go on forever.`
...
- [Check if Number is a Sum of Powers of Three](https://leetcode.com/problems/check-if-number-is-a-sum-of-powers-of-three/solutions/6436453/check-if-number-is-a-sum-of-powers-of-three/)
  hint: Prevent runaway recurssion for backtracking solution. Efficient solution: math greedy, refer to video. [Solution](https://www.youtube.com/watch?v=99ExTh_0Ycg)
- [Closest Dessert Cost](https://leetcode.com/problems/closest-dessert-cost/description/)
  hint: Be careful with edge cases. Notice: "closest" to target means exceeding target is allowed. [Solution](https://leetcode.com/problems/closest-dessert-cost/submissions/2117956795/)
### Tree
